...
function actualizarRegistro(id, patch){
  ...
  if(i>=0){ data[i]={...data[i],...patch}; guardarDatos(data); }
}
function ultimoRegistro(){
  return datos().find(r=>!r.eliminado) || null;
}
function registrosPor(codigo, estacion){
  return datos().filter(r=>!r.eliminado && String(r.codigo).trim()===String(codigo).trim() && r.estacion===estacion)
...
function tieneEvento(codigo, estacion, evento){
  return registrosPor(codigo,estacion).some(r=>String(r.evento||"").toUpperCase()===String(evento).toUpperCase());
}
