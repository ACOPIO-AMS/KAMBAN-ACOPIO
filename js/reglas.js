...
function eventoAutomatico(codigo, estacion){
  ...
  const usados=registrosPor(codigo,estacion).filter(r=>reglas.includes(String(r.evento||"").toUpperCase())).length;
  return reglas[usados] || "";
}

function validarSecuencia(codigo, estacion, evento){
  ...
  if(estacion==="CHANCADO"){
    ...
    if(ev==="PARADA" && ultimo==="PARADA"){
      return {ok:false,msg:"NO SE PUEDE REGISTRAR PARADA.\n\nYa está en PARADA. La siguiente lectura normal registrará REINICIO."};
    }
    if(ev==="FINAL" && ultimo==="PARADA"){
      return {ok:false,msg:"NO SE PUEDE REGISTRAR FINAL.\n\nPrimero escanee para registrar REINICIO."};
    }
  }
  ...
  if(estacion==="DESCARGUIO"){
    ...
    if(ev==="EN STOCK"){
      ...
      if(!yaFinal) return {ok:false,msg:"EN STOCK solo se permite después de FINAL en DESCARGUÍO."};
    }
    if(ev==="SALIDA STOCK" && st!=="EN STOCK"){
      return {ok:false,msg:"No se puede registrar SALIDA STOCK. El lote no está EN STOCK."};
    }
    if(st==="EN STOCK" && ev!=="SALIDA STOCK"){
      return {ok:false,msg:"LOTE EN STOCK. Primero registre SALIDA STOCK."};
    }
  }
...
function pendientesPorFinalizar(){
  ...
  return out.sort((a,b)=>{
    const p={"PARADA":1,"EN STOCK":2}; return (p[a.falta]||9)-(p[b.falta]||9) || String(a.ultimo).localeCompare(String(b.ultimo));
  });
}
